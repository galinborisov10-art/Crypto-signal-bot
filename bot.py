...
# Вземи базовата директория на проекта (където се намира bot.py)
...
async def admin_docs_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    readme_path = os.path.join(ADMIN_DIR, "README.md")
    ...
    if DIAGNOSTICS_AVAILABLE:
        try:
            ...
            result = subprocess.run(
                [sys.executable, os.path.join(ADMIN_DIR, 'diagnostics.py')],
                capture_output=True,
                text=True
            )
...
async def reports_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if BACKTEST_AVAILABLE:
        try:
            import os
            backtest_path = os.path.join(BASE_DIR, 'backtest_results.json')

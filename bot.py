...
# ================= BASE DIRECTORY =================
# Dynamic BASE_DIR based on script location
...
async def task_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        ...
        try:
            with open(os.path.join(BASE_DIR, 'copilot_tasks.json'), 'r') as f:
                import json
                data = json.load(f)
...
async def dailyreport_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Запази обратно
        with open(os.path.join(BASE_DIR, 'copilot_tasks.json'), 'w') as f:
            json.dump(data, f, indent=2, ensure_ascii=False)
        
        # Създай и файл с по-детайлна информация
        task_file = os.path.join(BASE_DIR, f"COPILOT_TASK_{new_task['id']}.md")
...
async def admin_docs_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    readme_path = os.path.join(BASE_DIR, "admin", "README.md")
...
async def test_system_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout.strip():
            ...
            subprocess.run([os.path.join(BASE_DIR, "bot-manager.sh"), "start"], timeout=30)
        ...
        if missing_modules:
            ...
            if result.returncode == 0:
                ...
                subprocess.run([os.path.join(BASE_DIR, "bot-manager.sh"), "restart"], timeout=30)
            else:
        ...
        if result.returncode == 0 and result.stdout.strip():
            ...
            subprocess.run([os.path.join(BASE_DIR, "auto-fixer-manager.sh"), "start"], timeout=30)
...
async def auto_update_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Автоматично обновява бота чрез update_bot.sh - само за admin"""
    user_id = update.effective_user.id
    ...
    await update.message.reply_text(
        "🔄 <b>АВТОМАТИЧНО ОБНОВЯВАНЕ</b>\n\n"
        "⏳ Изпълнявам update_bot.sh...\n"
        "Това ще:\n"
        "• Изтегли последните промени от GitHub\n"
        "• Инсталира зависимости\n"
        "• Почисти кеша\n"
        "• Рестартира бота чрез PM2\n\n"
        "⚠️ Ботът може да е недостъпен за няколко секунди.",
        parse_mode='HTML'
    )
    
    try:
        update_script = os.path.join(BASE_DIR, "update_bot.sh")
        
        # Провери дали скриптът съществува
        if not os.path.exists(update_script):
            await update.message.reply_text(
                "❌ update_bot.sh не е намерен!\n"
                f"Очакван път: {update_script}"
            )
            return
        ...
        if result.returncode == 0:
            ...
            await update.message.reply_text(
                f"✅ <b>ОБНОВЯВАНЕТО УСПЕШНО!</b>\n\n"
                f"<code>{output}</code>",
                parse_mode='HTML'
            )
        else:
            error_output = result.stderr[-1500:] if len(result.stderr) > 1500 else result.stderr
            await update.message.reply_text(
                f"❌ <b>ГРЕШКА ПРИ ОБНОВЯВАНЕ</b>\n\n"
        ...
        await update.message.reply_text(
            "⏱️ <b>TIMEOUT</b>\n\n"
            "Обновяването отне повече от 5 минути.\n"
            "Проверете логовете ръчно.",
            parse_mode='HTML'
        )
    except Exception as e:
        logger.error(f"Грешка при auto_update: {e}")
        await update.message.reply_text(
            f"❌ <b>ГРЕШКА</b>\n\n{str(e)}",
            parse_mode='HTML'
        )
...
async def reports_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if REPORTS_AVAILABLE:
        try:
            daily_reports_path = os.path.join(BASE_DIR, 'daily_reports.json')
            if os.path.exists(daily_reports_path):
                import json
                with open(daily_reports_path, 'r') as f:
                    data = json.load(f)
    ...
    if BACKTEST_AVAILABLE:
        try:
            backtest_results_path = os.path.join(BASE_DIR, 'backtest_results.json')
...
def main():
    ...
    app.add_handler(CommandHandler("auto_update", auto_update_cmd))  # Автоматично обновяване чрез update_bot.sh
